...
function insertNewsletterCTA(html) {
  const ctaEnabled = getSetting('cta.enabled', true);
  if (!ctaEnabled) {
    logger.info('CTA挿入: 無効（設定でOFF）');
    return html;
  }

  const ctaUrl = getSetting('cta.url', 'https://hinakira.net/p/r/RwKLzKtX');
  const shortLinkText = getSetting('cta.shortLinkText', 'AIを学び、使えるようになるメルマガ登録はこちら');
  const shortPrefix = getSetting('cta.shortPrefix', '▶ ');
  const detailedIntro = getSetting('cta.detailedIntro', '');
  const detailedBenefitsIntro = getSetting('cta.detailedBenefitsIntro', '');
  const detailedBenefits = getSetting('cta.detailedBenefits', []);
  const detailedOutro = getSetting('cta.detailedOutro', '');

  // --- 短いCTA（最初のh2の直前） ---
  const shortCTA = `<p>${shortPrefix}<a href="${ctaUrl}">${shortLinkText}</a></p>`;

  // --- 詳細CTA（記事末尾） ---
  const detailedParts = [];
  if (detailedIntro) detailedParts.push(`<p>${detailedIntro}</p>`);
  if (detailedBenefitsIntro) detailedParts.push(`<p>${detailedBenefitsIntro}</p>`);
  if (detailedBenefits.length > 0) {
    detailedParts.push('<ul>');
    for (const benefit of detailedBenefits) {
      detailedParts.push(`<li><strong>${benefit}</strong></li>`);
    }
    detailedParts.push('</ul>');
  }
  if (detailedOutro) detailedParts.push(`<p>${detailedOutro}</p>`);
  detailedParts.push(`<p>${shortPrefix}<a href="${ctaUrl}">${shortLinkText}</a></p>`);
  const detailedCTA = detailedParts.join('\n');
